...
export function GoogleSignInButton({ next = "/dashboard" }: { next?: string }) {
  ...
  async function handleClick() {
    ...
    // Browser is redirected to Google by the call above; loading state
    // stays true until that navigation happens.
  }
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className="cursor-pointer w-full inline-flex items-center justify-center gap-3 rounded-full border-2 border-(--color-ink-200) px-4 py-2.5 text-base font-semibold text-(--color-ink-900) hover:bg-(--color-ink-50) transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
    >
